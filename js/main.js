...
const listaPokemon = document.getElementById('listaPokemons');
const pokemoninfo = document.getElementById('infoPokemon2');
const dialogo = document.getElementById('texto-dialogo');
const pokemonspartida = document.getElementById('pokemonspartida');
const btnenviar = document.getElementById('btnenviar');
const btnborrar = document.getElementById('btnborrar');
const pokemonenpantalla = [];
const pokpartida = [];
const aleatorio = false;

pokemons.forEach(pokemon => {

    const celdapokemon = document.createElement("button");
    celdapokemon.classList.add("celda-pokemon");

    const nombrepk = document.createElement('p');
    nombrepk.classList.add('nombre-pokemon');
    nombrepk.innerText = pokemon.name;
    celdapokemon.appendChild(nombrepk);
    ...
    celdapokemon.appendChild(img);
    celdapokemon.appendChild(nombrepk);

    const contenedortipos = document.createElement('div');
    contenedortipos.classList.add('tiposPokemon');

    const tipopk1 = document.createElement("img");
    tipos.forEach(tipo => {
        if (pokemon.types[0] == tipo.type) {
            tipopk1.src = tipo.img;
            tipopk1.classList.add('tipos');
        }
    });
    ...
    tipos.forEach(tipo => {
        if (pokemon.types[1] == tipo.type) {
            tipopk2.src = tipo.img;
            tipopk2.classList.add('tipos');
            tienedostipos = true;
        }
    });
    if (tienedostipos) {
    ...
    celdapokemon.appendChild(contenedortipos);

    celdapokemon.addEventListener('click', () => {

        pokemonenpantalla.pop();
        pokemoninfo.innerHTML = "";

        const pknombre = document.createElement('h2');
        pknombre.innerText = pokemon.name;
        pknombre.classList.add("pknombre");

        const divtipos = document.createElement('div');
        divtipos.appendChild(tipopk1);
        divtipos.appendChild(tipopk2);
        divtipos.classList.add('pokemon-infotipos');

        const caracteristicas = document.createElement('div');
        const pokataque = document.createElement('p');
        const pokhp = document.createElement('p');
        const pokdefensa = document.createElement('p');

        pokataque.innerText = "Ataque: " + pokemon.attack;
        pokdefensa.innerText = "Defensa: " + pokemon.defense;
        pokhp.innerText = "HP: " + pokemon.hp;

        caracteristicas.appendChild(pokataque);
        caracteristicas.appendChild(pokdefensa);
        caracteristicas.appendChild(pokhp);
        caracteristicas.classList.add('caracteristicas');

        pokemoninfo.appendChild(pknombre);
        pokemoninfo.appendChild(img);
        pokemoninfo.appendChild(divtipos);
        pokemoninfo.appendChild(caracteristicas);

        pokemonenpantalla.push(pokemon);
    });

    listaPokemon.appendChild(celdapokemon);
});
...
btnenviar.addEventListener('click', () => {
    ...
    if (pokemonenpantalla[0] != null) {
        if (pokemonspartida.children.length >= 2) return;

        pokpartida.push(pokemonenpantalla[0]);
        
        const pokemonpartida = document.createElement('div');
        pokemonpartida.classList.add('pokemonpartida');

        const pokemonpartidaNombre = document.createElement('p');
        pokemonpartidaNombre.innerText = pokpartida[pokpartida.length - 1].name;
        pokemonpartida.appendChild(pokemonpartidaNombre);

        const pokimg = document.createElement('img');
        pokimg.classList.add('pokimg');
        pokimg.src = pokpartida[pokpartida.length - 1].img;
        pokimg.alt = pokpartida[pokpartida.length - 1].name;

        pokemonpartida.appendChild(pokimg);
        pokemonspartida.appendChild(pokemonpartida);

        dialogo.innerText = "Elige el Pokémon rival:" + pokpartida.length;
    }
});

btnaleatorio.addEventListener('click', () => {

    pokpartida.splice(0, pokpartida.length);
    pokemonspartida.innerHTML = "";

    const randomindex1 = Math.floor(Math.random() * 151);
    const randomindex2 = Math.floor(Math.random() * 151);

    pokpartida.push(pokemons[randomindex1]);
    pokpartida.push(pokemons[randomindex2]);
    ...
    for (let i = 0; i < 2; i++) {
        ...
        const pokemonpartidaNombre = document.createElement('p');
        pokemonpartidaNombre.innerText = pokpartida[i].name;
        pokemonpartida.appendChild(pokemonpartidaNombre);

        const pokimg = document.createElement('img');
        pokimg.classList.add('pokimg');
        pokimg.src = pokpartida[i].img;
        pokimg.alt = pokpartida[i].name;
...
btnborrar.addEventListener('click', () => {
    ...
    pokpartida.splice(0, pokpartida.length);
    pokemonspartida.innerHTML = "";
    dialogo.innerText = "Elige tu Pokémon:";
    pokemoninfo.innerHTML = "";
});
...
document.getElementById('iniciarCombate').addEventListener('click', () => {

    const tuPokemon = pokpartida[0];
    const rivalPokemon = pokpartida[1];

    setCookie('tuPokemon', tuPokemon, 1);
    setCookie('rivalPokemon', rivalPokemon, 1);
